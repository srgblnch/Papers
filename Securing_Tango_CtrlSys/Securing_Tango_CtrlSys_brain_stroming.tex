...
\newcommand{\version}{github.Papers: 2013-09-13\;(revision 2472c950) } %git log -n 1 --pretty=format:"%H"
...
\begin{itemize}
    \item \tango\, as an \emph{Industrial Control System} (ICS), that with some extra pieces becomes a \emph{Supervisory Control And Data Acquisition} (SCADA) (like \atk\, and also what \sardana\, and \taurus\, does, goes further). As will be explained in \ref{sec:further}, ensure \sardana, \taurus\, and \atk\, is out of the scope of this paper, except for those things that defines the interaction with \tango. Don't forget to mention the \mambo (the archiver).
...
%%%%
%
\subsection{Use examples \label{sec:useExamples}}

\begin{itemize}
    \item The example of a laboratory use: Optics Lab \emph{Long Term Profile} (LTP) for mirrors surface characterization in the angstr\"oms (1\AA{} = $1\times10^{-10}m$) range.
...
\bibliography{../bibtex/sblanch.bib,../bibtex/standards.bib,../bibtex/ecc.bib,../bibtex/rijndael.bib,../bibtex/isogeny.bib,../bibtex/books.bib,../bibtex/crypto.bib,../bibtex/rfc.bib,../bibtex/homomorphic.bib}
