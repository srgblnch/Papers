...
% \begin{itemize}
%     \item \todo{Link the information security with the security engineering using the fact that in distributed systems many machines are running in parallel and there is a very importan element: the trustiness of the machines where the agents runs (physical/network access)}
% \end{itemize}
...
%With this name is possible to access any public content of the agent. An attribute name is known in the system, in the short name, as {\tt domain/class/member/attr}.% and information about it, like the unit is known as {\tt domain/class/member/attr?configure=unit}

But how the user can contact a \emph{device} (or between agents), \tango\, solves this by a \emph{phone-book} in a \mysql\, database. But this solution leads to another problem: this data base is not integrated with in the \emph{middleware} resulting a centralized element.
...
Once said this points, it is time to go inside of the three proposed solutions for \tango.
...
\bibliography{../bibtex/sblanch.bib,../bibtex/books.bib,../bibtex/standards.bib,../bibtex/rfc.bib,../bibtex/tango.bib,../bibtex/crypto.bib,../bibtex/ecc.bib,../bibtex/isogeny.bib,../bibtex/symmetrics.bib,../bibtex/rijndael.bib,../bibtex/homomorphic.bib}
