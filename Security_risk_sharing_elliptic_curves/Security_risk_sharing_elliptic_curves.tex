...
\author{Sergi Blanch-Torn\'e\inst{1}, Ramiro Moreno Chiral\inst{2}, Francesc Seb\'e Feixa\inst{2}, Javier Valera Martin\inst{1}}
 \institute{
...
\todo{Currently there have been concerns about weaknesses in the curves of the NSA to NIST issued (those used in \cite{rfc6637}). Highlights questions about the computational efficiency by using curves with $a\equiv-3\;(mod\,p)$. Cite the references from D.J. Bernstein and Tanja Lange}
...
\todo{Compare the costs of those three algorithms (serial version), in time and memory. To do that make them equivalents, by adding to algorithm \ref{alg:verifiablyX9.62} the calculation of $n$ and $G$. Use the same than algorithm \ref{alg:verifiablyP1363}, because they look very similar}

\todo{What can be parallelize? Even that a key generation must be thought for the slowest case (an smardcard or an embedded system) it can be good to compare which is the advantage of parallelization.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Get an elliptic curve from a fixed cardinality and Finite Field}

\todo{Complex multiplication from P1363 (p.147)}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Get an elliptic curve from another good one \label{sec:from_another}}
...
\todo{Introduce the definition with a friendly text}
...
\begin{definition}\label{def:isogeny}
 ...
 Given two elliptic curves $E/\mathbb{K}$ and $E'/\mathbb{K}$, they are \emph{isogenies over $\mathbb{K}$} \emph{if and only if} exist an application between them that is a morphism between their corresponding sets of points over those curves.
...
\end{definition}
\fixme{Would be better to define what is an isogeny when the elliptic curve is over finite fields, because is much simpler and the more general definition over a field $\mathbb{K}$ is not providing any benefit. Review with the definition used in Ramiro's thesis (\cite{ramiro05} page 23) where is used the definition from [Hus04] Th12 (8.4)}
