...
%\newcommand{\version}{git: \gitCommitterDate (revision \gitAbbrevHash) }
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Then the fix polynomial element in the ring is always $$c(x) = (z+1)x^3+(1)x^2+(1)x+(z)$$. Remember that each coefficient is an element of a polynomial field, but this notation can be simplified by a binary notation: $c(x) = `0x3'y^3+`0x1'y^2+`0x1'y+`0x2'$. This polynomial element is coprime to the modulo ($x^4+1$) and therefore has an inverse in the ring used to revert the \emph{mixColumns}: $c(z) = `0xB'y^3+`0xD'y^2+`0x9'y+`0xE'$.
