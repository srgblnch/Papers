...
Before to enter in the design of the Rijndael schema is good to have some knowledge of the background used. This section can be avoided if the reader has already this skills, like can be recovered if some refresh can help. \fixme{Avoid to sound pedantic, or don't say that at all.}

The mathematics used just above the \emph{xor} operation are polynomial fields defined with coefficients in a binary group, denoted as
$$\Fpn{2}{n}=\frac{\mathbb{F}_{2}[z]}{m(z)}\label{eq:polynomialField}$$
Where the polynomial $m(z)$ is an irreductible with some given degree. As later will be used, for the notation of this degree we use $w$ and has correspondence with the \emph{Rijndael} schema of section \ref{sec:approach}.
...
\begin{enumerate}
 ...
 \item Inverse: $\forall a(z) \in \Fpn{2}{n}$ must $\exist a^{-1}(z) \arrowvert a(z)+a^{-1}(z) = n(z)$
 \item Commutative: $\forall a(z),b(z) \in \Fpn{2}{n}, a(z)+b(z)=b(z)+a(z)$
\end{enumerate}

This polynomial set has a second operation, we call it \emph{product} that has also an Abelian group structure. For convenience the \emph{addition} operation has $0(z)$ as identity (where all the coefficients are $0$) and \emph{product} has the polynomial $1(z)$ (where all the coefficients are $1$).

If one of those operations is an Abelian group, and the second one is non-commutative, if they satisfy the property:
\begin{enumerate}
 \item '*' Distributive: $\forall a(z),b(z),c(z) \in \Fpn{2}{n}, a(z)*(b(z)+c(z)=[a(z)+b(z)]*[a(z)+c(z)]$
\end{enumerate}
This is called a \emph{ring} $(R,+,*)$ and it's the field used in the operation of the \emph{subBytes} in section \ref{sec:subBytes}.

In the case of when those involved operations are Abelian groups, the this becomes what is called a \emph{field} $(K,+,*)$.

At this point is already clear the algebraic properties of the field described in \ref{eq:polynomialField}. But like this polynomial binary field used an underlying binary group, this field can be used as coefficients of a superpolynomial structure:
...
The fact that in this case the modular polynomial $m'(x)$ is a composited polynomial\footnote{say that is a \emph{composited polynomial} is the same than say that it is reducible. Being factorizable this polynomial it has an algebraic structure of a ring. In the case of an \emph{irreducible polynomial} what is get is an algebraic structure of a field} with some degree (not necessarily the same than in the polynomial field, in deed this we use \emph{c} to denote this degree because in the \emph{Rijndael} schema it depends on the number of columns).
...
\todo{What to say to link this (if everything is already said) to link with the next section}
...
\node [rectangle,draw] (rcon) at (.1,-5.7) {$1^i$ $\in$ \Fpnm{w}{z}};%{Rcon[i]};
\node [] (eq0) at (-1,-2.8) {=0};
\node [] (neq) at (.5,-3.6) {$\neq0$};
...
\todo{Introduce this section using the base of the section \ref{sec:math}}
