...
% TODO
\begin{itemize}
 \item \texttt{TODO:} key recovery attacks \cite{fullaes-192-256} and \cite{cryptoeprint:2009:241}. And attacks that theoretically breaks \emph{Rijndael} \cite{cryptoeprint:2009:374}, \cite{biclique-fullaes} (as said in \cite{arXiv:1210.7942} unpractical but theoretically breaks it).
 \item \texttt{TODO:} Like \emph{$3$-DES} helps the algorithm to \emph{survive} for a longer time, \emph{Rijndael} has something similar with the \emph{AESWrap} in the rfc3394 \cite{rfc3394}. (say it better, but it can be saw like a sextuple-AES: \emph{sequential multiple encryption}).
\end{itemize}
...
A pseudo-random permutation is used as a symmetric cryptosystem like Shannon have defined in ...\footnote{\texttt{FIXME:} review to be sure if it was in \cite{shannon-MathTheoryComm} or \cite{shannon-comTheorySecSys}, or both then cite the oldest}. Also Shannon have defined the concept of the \emph{perfect secrecy}
% FIXME: was in \cite{shannon-comTheorySecSys} or in \cite{shannon-MathTheoryComm} ??
...
\begin{itemize}
 \item Shannon: confusion \& diffusion $\Rightarrow$ substitution \& permutation \cite{shannon-MathTheoryComm} and \cite{shannon-comTheorySecSys}
 \begin{itemize}
  \item \texttt{TODO:} (a bit deep than what have said in the PRP, definition \ref{def:shannonPerfectSecrecy} about perfect secrecy.
 \end{itemize}
...
% TODO: from where the matrix comes from?
...
\begin{itemize}
 \item \texttt{TODO:} how to build new $S$ and $S^{-1}$ for $w\neq 8$.
 \begin{itemize}
  ...
  \end{itemize}
 \end{itemize}
 \item \texttt{TODO:} summarize the $S$ and $S^{-1}$ using $w=4$ and $w=2$ in their \emph{S-Box}es and their inverse Boxes.
\end{itemize}
...
where $m(x)$ is a composited polynomial\footnote{\texttt{FIXME:} composited polynomial is the same than say reducible? Because with an \emph{irreducible polynomial} we get a field, but if it factorized it's a ring} with the same degree than the $c$ columns parameter. This describes a polynomial ring. The coefficients of this polynomial ring are elements of a polynomial field $\Fpn{2}{n}$, where this notation is a shorter of
