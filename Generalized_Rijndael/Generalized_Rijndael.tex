...
\begin{definition}
 For a perfect secrecy system, the distributions of the ciphers between messages in the cipher space is \emph{computationally indistinguishable}\footnote{Denoted the meaning of computationally indistinguishable by the symbol $\approx_p$}:
 \begin{center}
...
\draw [blue,thick] (k-1-1.north west) rectangle (k-4-1.south east);
...
\node [] (eq0) at (-1,-2.8) {=0};
\node [] (neq) at (.5,-3.6) {$\neq0$};
\node [] (neq) at (k-1-1.north) {$i-c$};
\node [] (neq) at (k-1-5.north) {$i$};
\node [] (neq) at (k-1-6.north) {$i+1$};
% 
% %arrows to set into the algorithm
\draw [|->] (k-4-1.south) -- (-4.1,-5) -- (xor.west);
...
\end{tabular}
\end{center}}
\caption{Table of the number of rounds as function of block and key length}
\label{tab:nrounds}
\end{figure}

In the AES proposal of the Rijndael \cite{Daemen01aes-ammended} the number of rounds is described as a function of the block and the key length. The table of the figure \ref{tab:nrounds} is a copy of the one from the cited Rijndael document, in section 4.1. But in section 7.6 is said that this number has been determined by looking in to the most efficient attacks (known at that time) and adding a security margin.

Better is said in section 12.1 of the mention document where the number of rounds is described by a function:
\begin{equation}\label{eq:nrounds}
 N_r = max(N_k,N_b)+6
\end{equation}
And with this all the other possible alternatives can be interpolated.
