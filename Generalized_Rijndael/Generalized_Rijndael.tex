...
\todo{algorithm \ref{alg:keyExpansion}, has a operation {\tt Rcon[i/nColumns]} and it can be precalculated in a table or recursively generated:
\begin{equation}
    x(i) = x^i = x\cdot x^{i-1}=x\cdot x(i-1)
\end{equation}
with $x_1=2$ in \Fpn{2}{8}, but it must be generalized for \Fpn{2}{w}.}
