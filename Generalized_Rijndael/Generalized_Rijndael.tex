...
\node [rectangle] (fpnm) at (0,-2.5) {$ s(x),c(x),s'(x) \in$\Fpnm{w}{x} with $m(x)$ reductible and degree $c$};
\node [rectangle] (fpn) at (0,-3) {$s_{(i,j)} \in$\Fpnm{1}{z} with $m(z)$ irreductible and degree $w$};
