...
\begin{itemize}
 ...
 %\item \todo{key recovery attacks} \cite{fullaes-192-256} and \cite{cryptoeprint:2009:241}. And attacks that theoretically breaks \rijndael\, \cite{cryptoeprint:2009:374}, \cite{biclique-fullaes} (as said in \cite{arXiv:1210.7942} unpractical but theoretically breaks it). Section \ref{sec:attacks}.
 %\item \todo{Like \emph{$3$-DES} helps the algorithm to \emph{survive}} for a longer time, \rijndael\, has something similar with the \emph{AESWrap} in the rfc3394 \cite{rfc3394}. (say it better, but it can be saw like a sextuple-AES: \emph{sequential multiple encryption}). Section \ref{sec:aeswrap}
\end{itemize}
...
%   &&&&&&        &$z^7$&$+z^6$&$+z^5$&      &$+z^3$&      &$+z$&    &$=a(z)$\\
%   &&&&&&$\times$&     &$ z^6$&      &      &$+z^3$&$+z^2$&$+z$&$+1$&$=b(z)$\\
%   \hline
%   &&&&&&&$z^7$&$+z^6$&$+z^5$&      &$+z^3$&      &$+z$&    &\\
%   &&&&&&$z^8$&$+z^7$&$+z^6$&      &$+z^4$&      &$+z^2$&    &&\\
%   &&&&&$z^9$&$+z^8$&$+z^7$&      &$+z^5$&      &$+z^3$&    &&&\\
%   &&&&$z^{10}$&$+z^9$&$+z^8$&      &$+z^6$&      &$+z^4$&    &&&&\\
%   &$z^{13}$&$+z^{12}$&$+z^{11}$&      &$+z^9$&      &$+z^7$&    &&&&&&&\\
%   \hline
%    &$z^{13}$&$+z^{12}$&$+z^{11}$&$+z^{10}$&$+z^9$&$+z^8$&     &$+z^6$&      &      &      &$+z^2$&$+z$&        &$=p(z)$\\
%  \end{tabular}
% \end{equation}
...
% \fixme{Perhaps this remainder reduction is not clean enough and would be better to have the division details in here.}
% 
% \begin{equation}
%  \begin{tabular}{rrrrrrrrrrrrrrrrr}
%    $z^{13}$&$+z^{12}$&$+z^{11}$&$+z^{10}$&$+z^9$&$+z^8$&     &$+z^6$&      &      &      &$+z^2$&$+z$&    &    &$|$&$z^8+z^4+z^3+z+1$\\
%  \end{tabular}
% \end{equation}
...
%\fixme{This subsection is not clear at all. It must be refactored. Perhaps move the hexadecimal representation explanation here}
...
\begin{center}
 ...
 \scalebox{0.55}{\input{images/rijndaelDiagram.pgf}}
...
\caption{Block diagram of the iterative construction of the \emph{Rijndael Key Expansion} as a \emph{Pseudo Random Generator} (acronym PRG). }%\fixme{this figure has a mistake and is not corresponding with algorithm \ref{alg:keyExpansion}}}
...
In the book about the design of this algorithm \cite{Daemen:2002:DR:560131}, specially section 3.5 there is a summary table describing the number of rounds as a function of the key length:

\begin{equation}\label{tab:nrounds}
%\begin{center}
\begin{tabular}{|c||c|c|c|c|c|}
\hline
$N_r$     & $N_b = 4$ & $N_b = 5$ & $N_b = 6$ & $N_b = 7$ & $N_b = 8$ \\ \hline\hline
$N_k = 4$ &    $10$   &    $11$   &    $12$   &    $13$   &    $14$   \\ \hline
$N_k = 5$ &    $11$   &    $11$   &    $12$   &    $13$   &    $14$   \\ \hline
$N_k = 6$ &    $12$   &    $12$   &    $12$   &    $13$   &    $14$   \\ \hline
$N_k = 7$ &    $13$   &    $13$   &    $13$   &    $13$   &    $14$   \\ \hline
$N_k = 8$ &    $14$   &    $14$   &    $14$   &    $14$   &    $14$   \\ \hline
\end{tabular}
%\end{center}
\end{equation}

In the AES proposal of the Rijndael \cite{Daemen01aes-ammended} (section 4.1) this table is simplified to include only the sizes for the standard. But continuing to the section 7.6, there is an explanation about how these numbers are chosen. By looking in to the most efficient attacks (known at the time of those publications) and adding a security margin. Section 12.1 improves this description setting according to the function:
\begin{equation}\label{eq:nrounds}
 N_r = max(N_k,N_b)+6
\end{equation}
This function means that the number of rounds is the biggest number of columns between the block and the key, plus the security margin set as $6$. With the basic configuration of the \rijndael, those $N_b$ and $N_k$ steps means steps of $32$ bits, and can be summarized as each increase of $32$ bits (or decrease) adds (or reduces) on round.

Although this formula, there is no theoretical method to stablish which is the good number of rounds. This seems to be only an empirical approach based on the known attacks. As this paper tries to propose different sizes for the \rijndael, will be in section \ref{sec:parameterCombinations} about the parameter combinations where this number will be discussed.

Extrapolate a discrete section of parameter choice outside its boundaries would be a mistake because what can be true inside the region may not still valid outside it. Back to \cite{Daemen:2002:DR:560131}, two rounds of the \rijndael\, provides 'full diffusion', that is to say '\emph{every state bit depends on all the other state bits values from 2 rounds a go}' or in other words, change one bit on the origin will produce an effect on all the state bits. Then this equation \ref{eq:nrounds} can be extended to require $4$ rounds as a minimum for a 'full diffusion step' at the beginning and at the end of the cipher, but this is absorbed by the $6$ added.

\begin{itemize}
    \item \todo{In the original \rijndael, two rounds provides full diffusion, is this still valid changing those sizes?}
...
\begin{enumerate}
 %\item \todo{is it a \emph{Maximum Distance Separeble} (MDS) matrix? and/or a circulant matrix?}
...
\caption{Schematic diagram of the {\tt shiftRows()} transformation simplified to four rows and four columns. Colour coding indicates each cell to visualise the shift that can be also view with the sub-indexes.}%\fixme{This diagram must be improved, use indexes on the cells} \fixme{rows $0$ is shifted $0$ cells (in standard but this is an arbitrary decision)}}
\label{fig:shiftRows}
\end{center}
\end{figure}
