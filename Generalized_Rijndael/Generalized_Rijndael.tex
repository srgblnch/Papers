...
\begin{equation}\label{eq:rcon}
    x(i) = x^i = x\cdot x^{i-1}=x\cdot x(i-1)
\end{equation}
with an initial value of $x_1=2$ in \Fpn{2}{8}.

\fixme{{\tt Rcon} as with $x_1=2$ in \Fpn{2}{8} can be generalized for \Fpn{2}{w}?}
...
Looking on the schema of the figure \ref{fig:RijndaelDiagram} the first operation, skipping the first round where it is the {\tt addRoundKey()} with $k_0$ --that will be explain later in section \ref{sec:addRoundKey}-- is the {\tt subBytes()}. About what has been said in section \ref{sec:design} about Shannon's perfect secrecy, this operation skill is the \emph{confusion} because is a permutation. This transformation is a non-linear substitution of each word in the \emph{state} matrix. In the original Rijndael it is used a substitution table called \emph{S-Box}. This S-Box is represented in the figure \ref{tab:sbox8} and there is also an inverse of it in figure \ref{tab:inv-sbox8}.
...
\caption{Schematic diagram of the {\tt shiftRows()} transformation simplified to four rows and four columns.}%\fixme{This diagram must be improved, use indexes on the cells} \fixme{rows $0$ is shifted $0$ cells (in standard but this is an arbitrary decision)}}
\label{fig:shiftRows}
\end{center}
\end{figure}
