...
\begin{algorithm}
 ...
 \begin{algorithmic}[1]
  ...
  \FOR{$(j=0$;$j<nColumns$;$j++)$}
    ...
    \ENDFOR
  \ENDFOR
  \FOR{$(j=nColumns$;$j<(nColumns*(nRounds+1))$;$j++)$}
    \IF{j mod nColumns == 0}
      ...
      \ENDFOR
    \ELSIF{$(nColumns > 6)$ and $($j mod nColumns == 4$)$}
      \FOR{$(i=0$;$i<nRows$;$i++)$}
...
% TODO: rebuild with the fixed algorithm
\begin{figure}[h!]
\begin{center}
...
\matrix (k) [matrix of math nodes,nodes = {node style ge},%
        ...
        {
          \cdots &
          k_{s_{(0,0)}} & k_{s_{(0,1)}} & \cdots & k_{s_{(0,c)}} &
          k_{t_{(0,0)}} & k_{t_{(0,1)}} & \cdots & k_{t_{(0,c)}} &
          \cdots \\
          \cdots &
          k_{s_{(1,0)}} & k_{s_{(1,1)}} & \cdots & k_{s_{(1,c)}} &
          k_{t_{(1,0)}} & k_{t_{(1,1)}} & \cdots & k_{t_{(1,c)}} &
          \cdots \\
          \cdots &
          ...
          \cdots        \\
          \cdots &
          k_{s_{(r,0)}} & k_{s_{(r,1)}} & \cdots & k_{s_{(r,c)}} &
          k_{t_{(r,0)}} & k_{t_{(r,1)}} & \cdots & k_{t_{(r,c)}} &
          \cdots \\
        };
 %dashed boxes to represent the subparts of the generated key
 \draw [dashed] (k-1-2.north west) rectangle (k-4-5.south east);
 \draw [dashed] (k-1-6.north west) rectangle (k-4-9.south east);
 
 %boxes to indicate the steps of the algorithm
 \node [] (j) at (k-1-7.north) {$j$};
 \draw [red,thick] (k-1-7.north west) rectangle (k-4-7.south east);
 
 \node [] (j-1) at (k-1-6.north) {$j-1$};
 \draw [blue,thick] (k-1-6.north west) rectangle (k-4-6.south east);
 
 \node [] (j-c) at (k-1-3.north) {$j-c$};
 \draw [blue,thick] (k-1-3.north west) rectangle (k-4-3.south east);
 
 % if (j mod nColumns == 0)
 % w[0][j] := w[0][j-nColumns]$\oplus$SubBytes(w[1][j-1])$\oplus$Rcon[j/nColumns];
 \draw [] (k-1-3.north west) rectangle (k-1-3.south east);%w[0][j-c]
 \draw [] (k-2-6.north west) rectangle (k-2-6.south east);%w[1][j-1]
 \node [rectangle,draw] (subBytes) at (0,2.5) {subBytes()};
 \draw [->] (k-2-6.north west) -- (subBytes);
 \node [rectangle,draw] (rcon) at (0,4) {Rcon[$j/c$]};
 \node [circle,draw] (xor1) at (0,3.25) {};\draw [-] (xor1.north) -- (xor1.south);\draw [-] (xor1.west) -- (xor1.east);
 \draw [->] (subBytes) -- (xor1.south);
 \draw [->] (rcon) -- (xor1.north);
 \draw [->] (k-1-3.north east) -- (-1,3.25) -- (xor1.west);
 \draw [] (k-1-7.north west) rectangle (k-1-7.south east);%w[0][j]
 \node [diamond,draw,aspect=1.5] (jmodc) at (1.65,3.25) {j mod c};
 \draw [->] (xor1.east) -- (jmodc);
 \draw [->] (jmodc) -- (k-1-7.north east);
 \node [] (eq0) at (jmodc.south) {=0};
 
 % if (j mod nColumns == 0)
 % w[i][j] := w[i][j-nColumns]$\oplus$SubBytes(w[i+1 mod 4][j-1]);
 % else if (nColumns > 6) and (j mod nColumns == 4)
 % w[i][j] := w[i][j-nColumns]$\oplus$SubBytes(w[i][j-1]);
 % else
 % w[i][j] := w[i][j-nColumns]$\oplus$w[i][j-1];
 
 \node [rectangle,draw] (subBytes2) at (-1,-2.5) {subBytes()};
 \draw[|->] (k-4-6.south) -- (subBytes2.north);% w[x][j-1]
 

 \node [diamond,draw,aspect=2] (jmodc2) at (1.65,-4) {j mod c};%j mod nColumns
 \node (zero) at (jmodc2.west) {$=0$};
 \node (four) at (jmodc2.north west) {$=4$};
 \node (else) at (jmodc2.south) {else};
 
 % w[i][j-nColumns]$\oplus$SubBytes(w[i+1 mod 4][j-1]);
 \node [circle,draw] (xor0) at (-2,-4) {};\draw [-] (xor0.north) -- (xor0.south);\draw [-] (xor0.west) -- (xor0.east);
 \draw[|->] (k-4-3.south) -- (xor0.west);% w[x][j-nColumns]
 \draw[->] (subBytes2.south) -- (xor0.north);% 
 \draw[->] (xor0.east) -- (jmodc2.west);
 % w[i][j-nColumns]$\oplus$SubBytes(w[i][j-1]);
 \draw[->] (xor0.east) -- (jmodc2.north west);%TODO: The nC > 6;
 % w[i][j-nColumns]$\oplus$w[i][j-1];
 \node [circle,draw] (xor_else) at (-1,-5) {};\draw [-] (xor_else.north) -- (xor_else.south);\draw [-] (xor_else.west) -- (xor_else.east);
 \draw[|->] (k-4-6.south) -- (xor_else.north);% w[x][j-1]
 \draw[|->] (k-4-3.south) -- (xor_else.west);% w[x][j-nColumns]
 \draw[->] (xor_else.east) -- (jmodc2.south);
 
 \draw[->] (jmodc2.north) -- (k-4-7.south);
