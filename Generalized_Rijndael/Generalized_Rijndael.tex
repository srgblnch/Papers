...
To be used as an example, the parameters will be set up to be the same than the standardised \rijndael. The polynomial finite field will be $\Fpn{2}{8}$ where, from equation \ref{eq:polynomialField} $n=8$ and $m(z)=z^8+z^4+z^3+z+1$. For both operation in the field, the same ``random'' elements will be used:
...
Where this coefficient addition is an operation in the field $\mathbb{F}_{2}$ and corresponds to the very basic \emph{xor}.
...
\fixme{This tabular view is not cleaner than lineal without the detail within the product}

Then \emph{reduce} this $p(z)$ as \emph{modulo} $m(z)$
...
Start taking two ``random'' elements\footnote{As they are examples, the \emph{random} means simplicity and the coefficients has been chosen to have only 2 non-zero \emph{sub}coefficients.} from the polynomial ring:

\begin{equation}\label{eq:randomPolynomialRing}
 \begin{tabular}{rllll}
  $\alpha(x),\beta(x)$&$ \in_R (\Fpn{2}{8})^l$&&\\
  $\alpha(x)=$&$(z^7+z^5)*x^3$&$+(z^7+z^3)*x^2$&$+(z^7+z^6)*x$&$+(z^7+z^4)$\\
  $\beta(x)=$ &$(z^6+z^4)*x^3$&$+(z^5+z^4)*x^2$&$+(z^4+z^1)*x$&$+(z^7+z^3)$\\
 \end{tabular}
\end{equation}

If $l(x)$ divides $\beta(x)$ this element will not have an inverse and the operation will not be possible to undo.
...
\begin{equation}
 \alpha(x)*\beta(x)=(z^5 + z^4)*x^3 + (z^7 + z^6 + z^4)*x^2 + (z^6 + z^5 + z^4 + z^3 + z^2 +
z + 1)*x + z^7 + z^5 + 1
\end{equation}
